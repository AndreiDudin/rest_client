...
class RestClient:
    ...
    async def post(self, path: Any, **kwargs: Any) -> httpx.Response:
        return await self._send_request(method="POST", path=path, **kwargs)

    async def get(self, path: Any, **kwargs: Any) -> httpx.Response:
        return await self._send_request(method="GET", path=path, **kwargs)

    async def put(self, path: Any, **kwargs: Any) -> httpx.Response:
        return await self._send_request(method="PUT", path=path, **kwargs)

    async def delete(self, path: Any, **kwargs: Any) -> httpx.Response:
        return await self._send_request(method="DELETE", path=path, **kwargs)

    @allure_attach
    async def _send_request(self, method: Any, path: Any, **kwargs: Any) -> httpx.Response:
    ...
    @staticmethod
    def _get_json(rest_response: Any) -> dict:
        try:
